 ...
 * Core JavaScript: State Management, Mock Store, Navigation & UI Helpers
 * ==========================================================================
 */

// Global API Base URL (Spring Boot backend)
...
// ==========================================================================
// SEED DATA INITIALIZER (Stored in localStorage for seamless client demo)
// ==========================================================================
...
function initAppStorage() {
  if (!localStorage.getItem('smartshop_products')) {
    localStorage.setItem('smartshop_products', JSON.stringify(INITIAL_PRODUCTS));
  }
  ...
  if (!localStorage.getItem('smartshop_wishlist')) {
    localStorage.setItem('smartshop_wishlist', JSON.stringify([]));
  }
  if (!localStorage.getItem('smartshop_offers')) {
    localStorage.setItem('smartshop_offers', JSON.stringify(INITIAL_OFFERS));
  }
  if (!localStorage.getItem('smartshop_wallet_balance')) {
    localStorage.setItem('smartshop_wallet_balance', '5000');
  }
  if (!localStorage.getItem('smartshop_rewards_points')) {
    localStorage.setItem('smartshop_rewards_points', '1250');
  }
  if (!localStorage.getItem('smartshop_recent_views')) {
    localStorage.setItem('smartshop_recent_views', JSON.stringify([1, 2]));
  }
}

// Get all products
function getProducts() {
  const data = localStorage.getItem('smartshop_products');
  return data ? JSON.parse(data) : INITIAL_PRODUCTS;
}

// Get cart
function getCart() {
  const data = localStorage.getItem('smartshop_cart');
...
function getWishlist() {
  const data = localStorage.getItem('smartshop_wishlist');
  return data ? JSON.parse(data) : [];
}
...
// Update Badges for Cart and Wishlist
...
// Sync Wishlist heart icons on page
...
// Track recently viewed product for recommendation engine
function trackProductView(productId) {
  let views = JSON.parse(localStorage.getItem('smartshop_recent_views') || '[]');
  views = views.filter(id => id !== productId);
  views.unshift(productId);
  if (views.length > 10) views.pop();
  localStorage.setItem('smartshop_recent_views', JSON.stringify(views));
}
...
function showToast(title, message, type = "info") {
  ...
  toast.className = `smart-toast toast-${type}`;
...
function copyCoupon(code) {
  navigator.clipboard.writeText(code).then(() => {
    ...
    showToast("Coupon Code", `Use code: ${code}`, "info");
  });
}

// ==========================================================================
// AUTHENTICATION STATE & NAVBAR RENDERER
// ==========================================================================
function getCurrentUser() {
  const user = localStorage.getItem('smartshop_current_user');
...
function updateAuthNavbar() {
  ...
  if (user) {
    userNavContainer.innerHTML = `
      <div class="dropdown">
        <button class="btn nav-action-btn dropdown-toggle border-0 p-0" type="button" data-bs-toggle="dropdown" aria-expanded="false" title="Account">
          <div style="width: 40px; height: 40px; border-radius: 50%; background: var(--gradient-primary); color: #fff; display: flex; align-items: center; justify-content: center; font-weight: 700;">
            ${user.name.charAt(0).toUpperCase()}
          </div>
        </button>
        <ul class="dropdown-menu dropdown-menu-end shadow-lg border-0 rounded-3 mt-2 p-2" style="min-width: 220px;">
          <li class="px-3 py-2 border-bottom">
            <div class="fw-bold text-dark">${user.name}</div>
            <div class="small text-muted">${user.email}</div>
            <span class="badge bg-primary-subtle text-primary mt-1">${user.role || 'Member'}</span>
          </li>
          <li><a class="dropdown-item py-2" href="profile.html"><i class="fas fa-user-circle me-2 text-primary"></i> My Profile</a></li>
          <li><a class="dropdown-item py-2" href="orders.html"><i class="fas fa-box me-2 text-primary"></i> My Orders</a></li>
          <li><a class="dropdown-item py-2" href="finance.html"><i class="fas fa-wallet me-2 text-primary"></i> Smart Wallet</a></li>
          <li><a class="dropdown-item py-2" href="rewards.html"><i class="fas fa-gift me-2 text-primary"></i> Reward Points</a></li>
          ${user.role === 'ADMIN' ? '<li><a class="dropdown-item py-2 text-danger fw-bold" href="admin.html"><i class="fas fa-shield-alt me-2"></i> Admin Panel</a></li>' : ''}
          <li><hr class="dropdown-divider"></li>
          <li><a class="dropdown-item py-2 text-danger" href="javascript:void(0)" onclick="logoutUser()"><i class="fas fa-sign-out-alt me-2"></i> Logout</a></li>
        </ul>
      </div>
    `;
  } else {
    userNavContainer.innerHTML = `
      <a href="login.html" class="btn btn-outline-primary btn-sm px-3 rounded-pill fw-bold me-1">Login</a>
      <a href="register.html" class="btn btn-smart-primary btn-sm px-3 rounded-pill text-white fw-bold d-none d-sm-inline-block">Sign Up</a>
    `;
  }
}

function logoutUser() {
  localStorage.removeItem('smartshop_current_user');
  localStorage.removeItem('smartshop_jwt_token');
    ...
    <div class="col-12 col-sm-6 col-lg-3">
      <div class="product-card">
        <div class="product-image-container">
          ${isRecommendation 
            ? `<div class="badge-recommendation"><i class="fas fa-sparkles"></i> For You</div>` 
            : `<div class="badge-discount">${product.discount}</div>`
          }
          <button class="btn-wishlist-toggle" data-product-id="${product.id}" onclick="toggleWishlist(${product.id})" title="Save to Wishlist">
            <i class="far fa-heart"></i>
          </button>
          <a href="product-details.html?id=${product.id}" class="d-flex align-items-center justify-content-center w-100 h-100" onclick="trackProductView(${product.id})">
        ...
        <div class="product-body">
          ...
          <h3 class="product-title">
            <a href="product-details.html?id=${product.id}" onclick="trackProductView(${product.id})">${product.name}</a>
          </h3>
...
function loadHomepagePopularProducts() {
  ...
  container.innerHTML = popular.map(p => renderProductCard(p)).join('');
  updateWishlistIcons();
}
...
function loadHomepageRecommendedProducts() {
  ...
  const products = getProducts();
  // Simple rule-based initial recommendation: pick items from index 4 to 8
  const recommended = products.slice(4, 8).length > 0 ? products.slice(4, 8) : products.slice(0, 4);
  container.innerHTML = recommended.map(p => renderProductCard(p, true)).join('');
  updateWishlistIcons();
}
...
function loadHomepageOffers() {
  ...
  const offers = JSON.parse(localStorage.getItem('smartshop_offers') || '[]');
  container.innerHTML = offers.map(offer => `
    <div class="col-12 col-md-6 col-lg-3">
      <div class="offer-card ${offer.bgClass}">
        <div>
          <span class="offer-badge"><i class="fas fa-bolt me-1"></i> Hackathon Deal</span>
          <div class="offer-value">${offer.discount}</div>
          <h4 class="offer-title">${offer.title}</h4>
          <p class="offer-desc">${offer.description}</p>
        </div>
        <div class="coupon-box">
          <span class="coupon-code">${offer.code}</span>
          <button class="btn-copy-code" onclick="copyCoupon('${offer.code}')">Copy</button>
        </div>
      </div>
    </div>
  `).join('');
